...
export function ListUsedCarClient() {
  ...
  const [selectedBadges, setSelectedBadges] = useState<string[]>(["Hot Deal"]);
  ...
  function buildPayload(): CreateCarListingPayload {
    ...
    const pricing: CreateCarListingPayload["pricing"] = {
      retail,
      ...(promotional !== undefined && promotional > 0 ? { promotional } : {}),
      ...(fleet !== undefined && fleet > 0 ? { fleet } : {}),
      ...(parsedDown !== undefined && parsedEmi !== undefined && parsedDown > 0 && parsedEmi > 0
    ...
    return {
      type: "USED_CAR",
      title,
      description: fullDescription || `Pre-owned ${title}`,
      pricing,
      dealBadges: selectedBadges.filter((b) => DEAL_BADGE_OPTIONS.includes(b)),
      brand: derivedBrand,
      model: derivedModel,
      year: parseNumber(year) ?? 2024,
      condition: "USED",
      transmission,
      fuelType,
      ...
      ...(mileage ? { mileage: parseNumber(mileage) } : {}),
  ...
  async function processListingSubmission(shouldSubmit: boolean) {
    ...
    if (!retail || retail <= 0) {
      setError("Please enter a valid Retail Price for the used car.");
      return;
    }

    if (!model.trim()) {
      setError("Please enter a Model Name (e.g. Camry XSE, C300).");
      return;
    }
    ...
    try {
      ...
      console.log("[FORM SUBMIT] Creating used car listing payload:", JSON.stringify(payload, null, 2));

      const response = await supplierPortalApi.createCarListing(payload);
      const listingId = (response as { _id?: string; data?: { _id?: string } })?._id
      ...
      if (shouldSubmit) {
        ...
        setSuccessMessage("Used car listing created and submitted for admin review!");
      } else {
        setSuccessMessage("Used car listing saved as draft!");
      }
      ...
      console.error("[FORM SUBMIT ERROR]", err, errObj.payload);
      let msg = "Failed to save used car listing. Please check your inputs.";
      if (errObj.payload && "message" in errObj.payload) {
        const payloadMsg = errObj.payload.message;
        msg = Array.isArray(payloadMsg) ? payloadMsg.join(" | ") : String(payloadMsg);
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
            ...
            <TextField
              ...
              placeholder="e.g. Camry XSE, C300"
              value={model}
        ...
        <div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <TextAreaField
              ...
              placeholder="Provide information on vehicle condition, ownership history, service logs..."
              rows={4}
              value={description}
